...
export default function EditPhrasebookPage() {
  ...
  useEffect(() => {
    async function fetchItem() {
      try {
        const result = await fetchAPI(`/api/v1/phrasebook?limit=1`);
        const item = result.data.find((i: any) => i.id === id);

        if (item) {
          setFormData({
            ilokanoWord: item.ilokanoWord || "",
            englishTranslation: item.englishTranslation || "",
            tagalogTranslation: item.tagalogTranslation || "",
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-black">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-8">
          Suggest changes to this phrase. An admin will review before changes are published.
        </p>
